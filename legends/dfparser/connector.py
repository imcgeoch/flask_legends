""" Connector between the XML parser and the SQLAlchemy ORM.

Takes in mappings generated by the XML parser, runs their conversions,
...
class Connector():
    """The main object of the module. Connects to the database, consumes
    mappings, and writes them.
    """
    ...
    def add_world(self):
        """Writes the world to the database."""
        self.db.session.add(m.DF_World(id=self.world_id))
        self.db.session.commit()

    def add_mapping(self, xml_mapping):
        """Consumes a mapping from the XML parser into memory. If the
        configured memory limit has been reached, writes everything in
    ...
    def convert_mapping(self, xml_mapping):
        """Calls convert() on a mapping and stores the result in memory.

        returns the number of records that were stored.
        """
        xml_mapping.convert()
        if self.mode == 'update':
    ...
    def insert_mappings(self):
        """Inserts all stored mappings to the database."""
        s = self.db.session
    ...
    def update_mappings(self):
        """Updates the database with data from the stored mappings.
        Records must exist with the appropriate primary keys.
        """
        s = self.db.session
        mappings_to_insert = {}
    ...
    def mappings_to_db(self):
        """Calls insert_mappings and/or update_mappings, depending on
        the mode.
        """
        if self.mode == 'insert':
    ...
    def mapping_flawed(self, xml_mapping):
        """Checks whether the given mapping has issues that prevent
        proper processing.
        """
        return xml_mapping == {}
